...
char corner[16][4] =
    {" ", "╶", "╴", "─", "╷", "┌", "┐", "┬", "╵",
     "└", "┘", "┴", "│", "├", "┤", "┼"}; // its UTF-8 each character is of 3
                                         // bytes plus 1 byte for null character
////////////////////////////////////////////////////////////////////////////////////
...
void generate_maze(struct maze_context *maze) {
  ...
  if (maze->h <= 1 &&
      maze->w <= 1) { // 1x1 or less maze will make the code to go in an
                      // infinite loop so don't process those
    return;
  }
  ...
  while (1) {
    ...
    while (1) {
      ...
      cc = is_wall_visited(maze, r_y, r_x,
                           r_c); // it will return 1 if the neighbour next to
                                 // the wall is visited else 0
      all = is_all_visited(
          maze, r_y,
          r_x); // if all neighbours are visited it will return 1 else 0
      if (all)
        break;
    ...
    stack_insert(&stack_curr, r_y,
                 r_x); // insert the position of the first cell to the stack

    // if all neighbours visited backtrack to the cell which has at least one
    ...
    if (is_all_visited(maze, r_y, r_x)) {
      ...
      while (1) {
        stack_drop(&stack_curr); // drop the current cell from the stack and
    ...
    toggle_wall(maze, r_y, r_x, r_c,
                low); // torn down the wall between the current cell and the
                      // neighbour not visited
    struct cell *m;
    m = &maze->c[(r_y * maze_w) + r_x];
    ...
    if (maze->visit_count ==
        (maze_h * maze_w) - 1) { // if all cells visited end the loop
      break;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1) {
    ...
    printf("if no arguments are given a 15x20 maze will be generated by "
           "default\n");
    return 1;
  }

  struct maze_context *maze1 =
      (struct maze_context *)malloc(sizeof(struct maze_context));
