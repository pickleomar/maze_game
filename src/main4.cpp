 ...
 *raylib 5.0
 *
 *   Example contributed by @TheManTheMythTheGameDev and reviewed by Ramon
 *Santamaria (@raysan5)
 *
...
int main(void) {
  ...
  // Shadows are a HUGE topic, and this example shows an extremely simple
  // implementation of the shadowmapping algorithm, which is the industry
  // standard for shadows. This algorithm can be extended in a ridiculous number
  // of ways to improve realism and also adapt it for different scenes. This is
  ...
  Shader shadowShader = LoadShader(
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    // The idea is, we record all the objects' depths (as rendered from the
    // light source's point of view) in a buffer Anything that is "visible" to
    // the light is in light, anything that isn't is in shadow We can later use
    // the depth buffer when rendering everything from the player's point of
    // view to determine whether a given point is "visible" to the light
    ...
    DrawText("Shadows in raylib using the shadowmapping algorithm!",
...
void DrawScene(Model cube, Model robot) {
  ...
  DrawModelEx(cube, (Vector3){1.5f, 1.0f, -1.5f}, (Vector3){0.0f, 1.0f, 0.0f},
              0.0f, Vector3One(), WHITE);
  DrawModelEx(robot, (Vector3){0.0f, 0.5f, 0.0f}, (Vector3){0.0f, 1.0f, 0.0f},
              0.0f, (Vector3){1.0f, 1.0f, 1.0f}, RED);
}
